        ...
        <section className="px-6 md:px-10 pt-12 pb-20">
          <div className="max-w-6xl mx-auto">
            <div className="flex items-center gap-2 mb-4">
              <span className="text-xs font-semibold tracking-widest uppercase text-[#0a1628]/35">✦ ABOUT MERCURY</span>
            </div>
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-8">
              <div className="max-w-2xl">
                ...
                <p className="text-base text-[#0a1628]/50 leading-relaxed mb-8">
                ...
                <div className="flex flex-wrap gap-6">
                  {[{ v: "1990", l: "FOUNDED" }, { v: "30+", l: "CRAFTERS" }, { v: "1 Head City", l: "STUDIOS" }, { v: "1M+", l: "PIECES PRESSED" }].map((s) => (
                    <div key={s.l}>
                      ...
                      <p className="text-xs tracking-widest uppercase text-[#0a1628]/35 mt-1">{s.l}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {values.map((v) => (
                <div key={v.title} className="card p-7">
                  ...
                  <h3 className="font-bold text-[#0a1628] text-lg mb-2">{v.title}</h3>
                  <p className="text-sm text-[#0a1628]/45 leading-relaxed">{v.desc}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="flex justify-center gap-6">
              {team.map((t) => (
                <div key={t.name} className="group max-w-xs text-center">
                  <div className="rounded-2xl overflow-hidden mb-3 border border-black/10" style={{ aspectRatio: "4/5" }}>
                  ...
                  <p className="font-semibold text-[#0a1628] text-base">{t.name}</p>
                  <p className="text-xs text-[#0a1628]/40 mt-1">{t.role}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <Reveal delayMs={200}>
        <section className="px-6 md:px-10 mb-24">
          <div className="max-w-6xl mx-auto rounded-3xl p-10 md:p-16 text-center"
            ...
            <p className="text-sm text-[#0a1628]/45 mb-8 max-w-md mx-auto leading-relaxed">
            ...
            <div className="flex flex-wrap gap-3 justify-center">
              ...
              <Link href="/contact"
                className="flex items-center gap-2 text-sm font-medium px-8 py-3.5 rounded-full text-[#0a1628]/70 hover:text-[#0a1628] transition-colors"
